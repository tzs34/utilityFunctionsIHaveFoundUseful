...
const barSpacing = 65;
const unitPixels = 37.5; // 75
const axisConst = 187.5; // 375
...
class DivergeChart extends Component {
  ...
  renderBars = data => {
    if (data.length > 0) {
      return data.map(({ difference }, index) => {
  ...
  renderQuestions = (data, color, width) => {
    if (data) {
      return data.map(({ question }, index) => {
        let top = index > 0 ? barSpacing * (index + 1) : barSpacing;
        return (
          <QuestionContainer top={top} key={index} width={width}>
  ...
  render() {
    let { width, data: { values } = [] } = this.props;
    let { renderBars, renderQuestions } = this;
    let year = new Date().getFullYear();

    let filtered = values.filter(o => o.difference !== 0).sort((a, b) => {
            ...
            <Chart>
              <ChartColumn>
                <SmallContainer>
                  <Overlay>
                    <SmallContainer>
                      {renderQuestions(filtered, tmGrey, width)}
                    </SmallContainer>
                  </Overlay>
                </SmallContainer>
              </ChartColumn>
              <ChartColumn>
                <ChartBackground>
                  <BackgroundDiv size={75} color={tmDCDkPink} left={0} />
                  <BackgroundDiv size={75} color={tmDCMidPink} left={75} />
                  <BackgroundDiv size={37} color={tmDCLtPink} left={150} />
                  <XAxis color={tmMidBlack} left={184} />
                  <BackgroundDiv size={37} color={tmDCLtGreen} left={187} />
                  <BackgroundDiv size={75} color={tmDCMidGreen} left={225} />
                  <BackgroundDiv size={75} color={tmDCDkGreen} left={300} />
                  <Overlay>
                    <LargeContainer>{renderBars(filtered)}</LargeContainer>
                    <LargeContainer>
                      <YAxisContainer>
                        <AverageContainer>
                          <XAxisLabel>
